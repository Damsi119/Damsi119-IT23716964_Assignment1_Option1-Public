...
DEFAULT_EXCEL_CANDIDATES = [
    str(TESTS_DIR / "IT23716964_Assignment 1 - Test cases.xlsx"),
...
DEFAULT_INPUT_LENGTH_TYPE_COLUMN_CANDIDATES = [
    "Input length type",
    "Input Length Type",
    "Length Type",
    "Input length",
...
def _tokenize_header(value) -> set[str]:
    if value is None:
        return set()
    return {t for t in re.split(r"[^a-z0-9]+", str(value).strip().lower()) if t}
...
def _find_column_index(header_values: list, requested_name: str | None, candidates: list[str]) -> int | None:
    ...
    for item in indexed:
        ...
        if n and n not in norm_to_index:
            norm_to_index[n] = item["idx"]

    def match(name: str, allow_fuzzy_substring: bool = True) -> int | None:
        ...
        target_tokens = _tokenize_header(name)
        if target_tokens:
            for item in indexed:
                if item["tokens"] == target_tokens:
                    return item["idx"]
            for item in indexed:
                if target_tokens.issubset(item["tokens"]):
                    return item["idx"]

        if not allow_fuzzy_substring:
            return None
    ...
    for c in candidates:
        found = match(c, allow_fuzzy_substring=False)
        if found:
            return found

    for c in candidates:
        found = match(c, allow_fuzzy_substring=True)
        if found:
            return found
...
def _classify_input_length_type(text: str) -> str:
    size = len(text or "")
    if size <= 30:
        return "S"
    if size <= 299:
...
def _parse_args():
    ...
    parser.add_argument("--input-length-col", default=None)
...
def run_test():
    ...
    input_length_col_idx = _find_column_index(
        header_values, args.input_length_col, DEFAULT_INPUT_LENGTH_TYPE_COLUMN_CANDIDATES
    )
    ...
    if args.input_length_col and not input_length_col_idx:
        input_length_col_idx = _ensure_column(ws, header_row, header_values, args.input_length_col)
    ...
    with sync_playwright() as p:
        ...
        for row_index in range(header_row + 1, int(ws.max_row or 0) + 1):
            ...
            if not singlish_input:
                continue
            if input_length_col_idx:
                _set_cell_value(ws, row_index, input_length_col_idx, _classify_input_length_type(singlish_input))
